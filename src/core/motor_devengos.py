...
# --- CONSTANTES LEGALES SE REMUEVEN POR DEUDA TÉCNICA ---
# El SMMLV y Auxilio se inyectarán dinámicamente desde base_legal.py para evitar sesgos de año fiscal
...
def calcular_deducciones_ley(ibc, smmlv, porcentajes_ss):
    """
    Aportes obligatorios a Seguridad Social (Salud y Pensión).
    Aplica el tope máximo de cotización de 25 SMMLV.
    """
    tope_25 = smmlv * 25
    ibc_final: float = min(ibc, tope_25)
    salud = round(ibc_final * porcentajes_ss["empleado_salud"], 0)
    pension = round(ibc_final * porcentajes_ss["empleado_pension"], 0)
    return salud, pension

def obtener_tarifa_fsp(ibc, smmlv):
    """
